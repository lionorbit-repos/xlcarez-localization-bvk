import 'package:XLcarez/env/appexports.dart';

class Orgaadminlanding extends StatefulWidget {
  const Orgaadminlanding({super.key});

  @override
  State<Orgaadminlanding> createState() => _OrgaadminlandingState();
}

class _OrgaadminlandingState extends State<Orgaadminlanding> {
  int index = 0;
  @override
  Widget build(BuildContext context) {
    final isfab = MediaQuery.of(context).viewInsets.bottom != 0;
    return Networkcheckwrapper(
        child: MyThemeGradient(
      child: Scaffold(
        body: _items[index]["page"] as Widget,
        bottomNavigationBar: Visibility(
          visible: !isfab,
          child: BottomAppBar(
            height: 80,
            color: bgClr1,
            surfaceTintColor: bgClr1,
            clipBehavior: Clip.antiAlias,
            shape: const CircularNotchedRectangle(),
            notchMargin: 0.01,
            child: BottomNavigationBar(
                backgroundColor: bgClr1,
                type: BottomNavigationBarType.fixed,
                elevation: 0,
                selectedItemColor: primary,
                unselectedItemColor: hintClr,
                currentIndex: index,
                onTap: (val) {
                  setState(() {
                    index = val;
                  });
                },
                items: _items
                    .asMap()
                    .entries
                    .map((r) => BottomNavigationBarItem(
                        activeIcon: null,
                        icon: r.value["icon"] as Widget,
                        label: r.value["label"] as String))
                    .toList()),
          ),
        ),
      ),
    ));
  }

  String? fetchlabels(key) {
    var label = {
      0: fetchlocale(context).home,
      1: fetchlocale(context).userse,
      2: fetchlocale(context).profile,
    };
    return label[key];
  }

  final _items = [
    {
      "icon": const ImageIcon(
        AssetImage(bottom1),
        size: 18.0,
      ),
      "page": const Admindashboard(),
      "label": "Home"
    },
    {
      "icon": const ImageIcon(
        AssetImage(bottom9),
        size: 18.0,
      ),
      "page": const Userview(),
      "label": "Users"
    },
    {
      "icon": const ImageIcon(
        AssetImage(bottom9),
        size: 18.0,
      ),
      "page": const ProfileScreen(),
      "label": "Profile"
    },
  ];
}
